...
移動中にスマホで「〜して」と投げれば、Mac で無人処理して結果を返す。
...
- **cmux と同期**: 手元のターミナル（cmux）で開いている作業セッションに Slack から**そのまま連携**でき、新規の作業は**タブが増えて UI からも見える**（cmux 必須）。
...
2. `config.sample.yml` をコピーし、**プロジェクト共通の設定**を記入して配る（トークンは入れない）:
   - `repos`（対象リポジトリ。複数なら AI が指示から作業先を振り分ける）
...
```
READMEのタイポを確認して報告して        ← cmux にタブが立ち、無人処理して通知が届く（👀 が付けば受信OK）
```
...
| 入力（DM） | 動作 |
|---|---|
| `<自然言語の指示>` | タスク実行（既存 cmux セッションに連携 or 新規タブで起動）。プレフィックス不要 |
| 進行中の作業の話題を書く／画面の文面を貼る | いま cmux で動いているセッションの話題と噛み合えば**そこに合流**（画面の内容で照合する）。該当が無ければ新規タブ |
| `/sessions` | 動いているセッションを一覧（番号を返信するとそのタブに合流）。ID を知らなくてよい |
| `workspace_id=<UUID> <指示>` | cmux のタブを**直接指名**して合流（照合を通さない）。`cmux://workspace/<UUID>` でも可。ID は合流先のタブで `cmux identify --id-format both` |
| スレッドで続けて発言 | 追加指示・質問は同じセッション宛に届く（状況もそのスレッドで聞ける） |
| タスクスレッドで「やめて」 | そのセッションに割り込んで中断 |
| `/model fable` `/mode auto` | モデル/動作モードの変更（引数なしなら番号ピッカー） |
| `/commit` `/push` | そのスレッドのタスクを commit / push |
| `/status` | このスレッドの紐付け（セッション・作業先・claude の生死）を表示 |
| `/unlink` | このスレッドとセッションの紐付けを解除（誤合流の修正） |
